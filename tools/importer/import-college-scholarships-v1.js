...
/*
 * import-college-scholarships-v1 — dedicated importer for the USTA Foundation
 * what-we-do/college-scholarship-opportunities.html (general template).
 *
 * Section sequence (measured on the live rendered DOM at 1280):
 *   1. HERO (text-up) — bg scholarships-header.gif + H1 "We give young people the
 *      keys to their future." + subhead + one CTA "LEARN MORE" (pdf).
 *   2. "Opening doors of opportunity." — intro (center, medium) + Cards (content)
 *      ×3 (College Launch / College Success / Novo Nordisk Donnelly), image + h4 + desc.
 *   3. YELLOW BAND — "College Launch Scholarship" center-intro + a 4-up Q&A detail
 *      grid (Cards (content), NO images: h4 question + answer ×4).
 *   4. "College Success Scholarship" center-intro + 4-up Q&A detail grid.
 *   5. YELLOW BAND — "Novo Nordisk Donnelly Scholarship" center-intro + 4-up Q&A grid.
...
// Make a clean <img> from a source URL (absolutized), preserving alt.
...
// Emit a Cards (content) block from [{title, desc, img, alt}] defs. A card WITH an
// image is [imageCell, bodyCell]; a card with NO image is body-only [bodyCell] (the
// scholarship Q&A grids are image-less h4+answer cards).
...
export default {
  transform: (payload) => {
    ...
    // A cards section's INTRO (heading + lead para(s)).
    ...
    // 2. "Opening doors of opportunity." intro + 3 scholarship cards (with images).
    ...
    // 3–5. Three scholarship DETAIL sections: a centered heading + lead paragraph,
    // then a 4-up image-less Q&A grid (Cards (content): h4 question + answer). Two
    // sit on yellow bands (Launch, Novo Nordisk); College Success is white.
    const DETAILS = [
      {
    ...
    // SECTION 2 — "Opening doors of opportunity." intro (center, medium) + 3 cards.
    ...
    // SECTIONS 3–5 — scholarship detail sections: centered heading + lead para +
    // a 4-up image-less Q&A grid. Yellow bands (Launch, Novo Nordisk) get a leading
    // yellow strip; all use center-intro to centre the leading heading + lead.
    DETAILS.forEach((det) => {
      main.append(document.createElement('hr'));
      if (det.yellow) {
